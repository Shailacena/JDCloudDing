...
func UpdateMerchantBalance(c echo.Context, db *gorm.DB, o *model.Order) error {
	...
	err := repository.Merchant.UpdateBalance(db, o.MerchantId, o.OrderId, -o.ReceivedAmount, model.BalanceFromTypeOrderDeduct)
	if err != nil {
		return err
	}
...
func updateOrder(c echo.Context, db *gorm.DB, orderId string, jsonData JSONData) error {
	...
	err = repository.Order.Update(db, orderId, model.Order{
